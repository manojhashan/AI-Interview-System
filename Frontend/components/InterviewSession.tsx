...
const InterviewSession: React.FC<InterviewSessionProps> = ({
  ...
  const [step, setStep] = useState<
    "selection" | "setup" | "active" | "analyzing" | "finished"
  >("selection");
  ...
  // Audio recording for vocal model
  const audioMediaRecorderRef = useRef<MediaRecorder | null>(null);
  const audioChunksRef = useRef<Blob[]>([]);

  // WAV encoder — converts Web Audio API decoded buffer to WAV Blob
  ...
  const convertBlobToWavBase64 = async (webmBlob: Blob): Promise<string> => {
  ...
  const startSetup = async () => {
    ...
    try {
      ...
      // Get VIDEO-ONLY stream for the camera preview
      ...
      try {
        ...
        // Fallback: try without resolution constraints (some webcams reject exact dims)
        try {
          videoStream = await navigator.mediaDevices.getUserMedia({
            video: true,
            audio: false,
          ...
          throw new Error(`Camera Error: ${camErr?.message || "Permission denied or camera not found"}. Please allow camera access in your browser and try again.`);
        }
      }
      ...
      // Get AUDIO-ONLY stream separately — never assigned to any muted element
      ...
      try {
        ...
        throw new Error(`Microphone Error: ${micErr?.message || "Permission denied or mic not found"}. Please allow microphone access in your browser and try again.`);
      }

      // Explicitly check for audio track
      const audioTracks = audioStream.getAudioTracks();
      if (audioTracks.length === 0 || !audioTracks[0].enabled) {
        throw new Error("Microphone not detected or disabled. Please connect a microphone and try again.");
      }
  ...
  const startSpeechRecognition = () => {
    ...
    // Create a FRESH instance every time (never reuse — reuse causes "aborted" errors)
    ...
    recognition.onerror = (event: any) => {
      ...
      // no-speech / aborted → just let onend handle restart
    };
  ...
  const handleStartRecording = () => {
    if (isRecordingRef.current) return; // Prevent double clicks
    ...
    // Start audio recording for vocal model
    if (audioStreamRef.current) {
    ...
    // Wait for mic to fully unmute before starting recognition
    setTimeout(() => {
      if (isRecordingRef.current) {
        startSpeechRecognition();
      }
      ...
      <div className="max-w-4xl mx-auto py-10 animate-in fade-in slide-in-from-bottom-4">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400">
            Select which resume profile you'd like to use for this session.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {user.resumes.map((resume) => (
            ...
            >
              <h3
                className={`text-xl font-bold mb-2 ${selectedResume?.id === resume.id ? "text-white" : "text-slate-200"}`}
              >
                {resume.resumeTitle}
              </h3>
              <p
                className={`text-xs uppercase tracking-widest font-bold mb-8 ${selectedResume?.id === resume.id ? "text-blue-200" : "text-slate-500"}`}
              >
                Professional Profile
              </p>

              <div
                className={`space-y-2 text-sm ${selectedResume?.id === resume.id ? "text-blue-100" : "text-slate-400"}`}
              >
    ...
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 max-w-2xl mx-auto text-center animate-in zoom-in duration-300">
        ...
        <p className="text-slate-500 mb-8">
          Role: {selectedResume?.resumeTitle}
        </p>
        ...
        <div className="flex justify-center gap-4 mb-8">
          <div className="flex items-center gap-2 px-4 py-2 bg-slate-800 rounded-lg">
            <Camera
              size={18}
              className={
                mediaStreamRef.current ? "text-emerald-400" : "text-red-400"
              }
            />
          ...
          <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-6">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-sm w-full shadow-2xl animate-in zoom-in duration-200">
              ...
              <p className="text-slate-400 text-center text-sm mb-8">
                Are you sure? Your progress in this session will be lost and
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl">
              <div className="flex items-center gap-3 mb-4">
                <span className="bg-blue-600/20 text-blue-400 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-widest">
                  Question {currentQuestionIdx + 1} of {questions.length}
                </span>
                <span className="bg-slate-800 text-slate-400 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-widest">
                  {q?.category}
                </span>
              </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl h-full flex flex-col justify-center text-center space-y-8">
              <p className="text-slate-400 text-sm">
                Use the controls below to answer. AI analyzes your behavior in
                real-time.
              </p>
              ...
              {isProcessing && (
                <div className="p-4 bg-slate-800 rounded-xl flex items-center justify-center gap-3 animate-pulse">
                  ...
                  <span className="text-blue-400 font-medium">
                    AI Multimodal Engine Analyzing...
                  </span>
                </div>
              )}
            </div>
  ...
  return (
    <div className="max-w-md mx-auto text-center py-20 animate-in fade-in duration-500">
      ...
      <p className="text-slate-400 mb-10">
        You have completed all {questions.length} questions for the **
        {selectedResume?.resumeTitle}** role.
      </p>
      <button
